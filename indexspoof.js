...
if (sliderBar && sliderCurrent && movieCard) {
  sliderBar.oninput = function () {
    ...
    const filtered = (moviesData?.Search || []).filter(
      (n) => parseInt(n.Year, 10) <= parseInt(this.value, 10)
    );
    if (filtered.length > 0) {
      const movieCardHtml = filtered
        .map((movie) => {
      ...
      movieCard.innerHTML =
        "<p>No movies found for the selected year range.</p>";
    }
  };
}
